...
/**
 * El perfil del alumno, traducido a instrucciones.
 *
 * Va sin nombre, sin correo y sin identificador: son señales pedagógicas, no una
 * ficha. «Sacó 0 de 8 en asincronía» calibra la pregunta igual de bien que un
 * nombre y no expone a nadie ante un tercero.
 */
function segunElPerfil(perfil) {
  if (!perfil) return [];
  const l = [];
  const nivel = {
    base: 'Este alumno viene costándole. Haz la pregunta directa y sobre lo esencial '
        + 'del concepto: que pueda acertar si entendió la idea, sin trampas ni matices finos.',
    media: 'Pregunta de dificultad normal: hay que haber entendido el concepto, no basta '
         + 'con haber leído la palabra.',
    alta: 'Este alumno viene acertando casi todo. Súbele la exigencia: que tenga que '
        + 'aplicar el concepto a una situación, no solo reconocer su definición.',
  }[perfil.dificultad];
  if (nivel) l.push('', nivel);

  const debiles = (perfil.secciones_debiles ?? [])
    .map((d) => `${d.seccion} (${d.obtuvo} de ${d.de})`);
  if (debiles.length) {
    l.push('',
      `En el diagnóstico de entrada quedó bajo el umbral en: ${debiles.join('; ')}.`,
      'Si el concepto se conecta con alguna de esas áreas, apóyate ahí para reforzarla.');
  }

  // Que el perfil calibre la pregunta, no que aparezca en ella.
  l.push('', 'IMPORTANTE: no menciones su desempeño, sus notas ni sus debilidades en el',
    'texto de la pregunta ni en la explicación. Eso es para que calibres, no para decírselo.');
  return l;
}

/**
 * Dónde está parado el curso.
 *
 * Sin esto el modelo escribe preguntas correctas pero desubicadas: no sabe si el
 * ramo es de segundo o de sexto semestre, con qué herramientas se trabaja ni en
 * qué clase van. Una pregunta sobre «log» redactada sin saber que este curso
 * llega a Kafka en noviembre puede apuntar a cualquier parte.
 *
 * Lo que **no** se le pasa es la clase que viene. Si la conoce, tiende a colar
 * materia que todavía no se ha dictado.
 */
function elCurso({ contexto, clase_actual, recorrido }) {
  const l = [];
  if (contexto) l.push('', 'Sobre el curso:', contexto);
  if (clase_actual) {
    l.push('', `Esta semana el curso va en la clase ${clase_actual.codigo}: `
             + `«${clase_actual.titulo}».`);
  }
  const vistas = (recorrido ?? []).map((c) => `${c.codigo} ${c.titulo}`);
  if (vistas.length) {
    l.push(`El alumno ya vio: ${vistas.join('; ')}.`,
      'No preguntes por materia que no esté en esa lista, aunque la conozcas.');
  }
  return l;
}

export function instruccion({ termino, definicion, asignatura, fuente, perfil,
                              contexto, clase_actual, recorrido }) {
  return [
    `Eres ayudante de un curso de ${asignatura} en Duoc UC, Chile.`,
    ...elCurso({ contexto, clase_actual, recorrido }),
    '',
