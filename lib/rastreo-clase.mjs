...
export const VERSION_RASTREO = 4;
...
(() => {
  ...
  // `cambiarModo()` termina llamando a `guardar()`, así que si la intercepción
  // se instalara después, ese primer guardado se perdería; y `slideMax` y
  // `respuestas` son `let`, así que una intercepción que se disparara antes de
  // sus declaraciones caería en la zona muerta y `mirar()` lanzaría.
  ...
  /* ---------- 4. Modo estudio ---------- */
  // El deck arranca en modo `clase`, pensado para proyectar, y en ese modo
  // `guardar()` sale temprano y no persiste nada. Sin esto no habría avance que
  // reportar y nadie sumaría un punto.
  //
  // `cambiarModo` es una declaración de función en el nivel superior del script
  // del deck, así que queda en el objeto global y este script —que va después—
  // la alcanza. Si algún día no estuviera, el respaldo es el botón del HUD, que
  // alterna: por eso solo se pulsa cuando el modo actual es `clase`.
  try {
