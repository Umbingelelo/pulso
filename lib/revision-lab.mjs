 ...
 * ── La pauta ──
 *
 * Si el laboratorio trae pauta para esa caja, va también. Es la respuesta que el
 * docente escribió al publicar la guía, y sirve para lo que el enunciado solo no
 * puede: en las cajas conceptuales —«¿por qué esa firma no sirve de nada?»— el
 * modelo tenía que reconstruir el criterio en cada llamada, y dos alumnos que
 * escribieron lo mismo podían salir con veredictos distintos.
 *
 * **La pauta no se le muestra al alumno nunca**, ni se le manda al navegador: vive
 * en una columna que `mi_laboratorio()` no selecciona y la trae `/api/laboratorio`
 * por su propio camino. Ver `0030_pauta.sql`.
 *
 * Y tenerla adentro empeora el riesgo que ya existía —que el modelo suelte la
 * respuesta en el mensaje— porque ahora la tiene escrita al lado. Por eso la
 * prohibición de soplar aparece dos veces en la instrucción: una vez donde se
 * entrega la pauta y otra donde se explica cómo escribir el mensaje. Y por eso
 * `probar-revision.mjs` mide justamente eso.
 *
 ...
 * lo que el alumno pegó corresponde a **ese** paso y no a otro. La pauta no
 * cambia eso: es una referencia para comparar, no una plantilla que calzar.
 *
...
const ESQUEMA = {
  ...
  properties: {
    ...
    mensaje: {
      type: 'string',
      // El mínimo no es cosmético. Sin él, el modelo devolvió una vez un
      // veredicto con el mensaje **en blanco**, y eso pasa la validación de
      // tipos sin problema: el alumno recibe un «Vuelve a mirarlo» y un recuadro
      // vacío debajo, que es peor que no haber pedido la sugerencia. Se cierra
      // en los dos lados —acá y en `revisar()`— porque el proveedor no siempre
      // respeta `minLength`.
      minLength: 20,
      description: 'Dos o tres frases, en español de Chile, tuteando al alumno. ' +
        'Nunca la respuesta: qué mirar o qué revisar. Nunca vacío.',
    },
  },
};
 ...
 *
 * `pauta` es opcional: hay laboratorios publicados sin ella y el criterio de
 * abajo funciona igual, solo con menos apoyo.
 */
export function armarInstruccion(lab, cajaId, respuesta, pauta) {
  ...
  // El bloque de la pauta se arma aparte porque cuando no hay, no tiene que
  // quedar ni un encabezado vacío: un «PAUTA: (ninguna)» invita al modelo a
  // comentar su ausencia, y eso no es asunto del alumno.
  const bloquePauta = (pauta ?? '').trim()
    ? `
═══════ LA PAUTA DE LA CAJA ${cajaId} · ES PARA TI, NO PARA ÉL ═══════
Esto es lo que el docente considera una respuesta correcta a esta caja. Úsala
para decidir el veredicto: es el criterio con el que se juzgan todos los alumnos.

Tres cosas sobre cómo usarla:

1. **No la cites, no la parafrasees, no la resumas, ni entera ni por partes.**
   Tenerla escrita acá al lado no cambia la regla del mensaje: si se la entregas,
   el alumno no aprendió y encima puede escribírtela de vuelta. Es el error más
   fácil de cometer teniendo este texto delante.
2. **No exijas que la respuesta se parezca a la pauta.** La pauta dice el fondo;
   el alumno lo va a decir con otras palabras, más corto y en otro orden, y eso
   está bien. Lo que se compara es la idea, no el texto.
3. **La pauta no manda por sobre el enunciado.** Si la caja pide dos cosas y la
   pauta menciona una, sigue pidiendo las dos. Y si la pauta cubre algo que la
   caja no pidió, no lo exijas.
...
- «logrado»: capta la idea **y entrega lo que la caja pide**. Puede estar dicho con
  torpeza; lo que no puede es faltarle algo que la caja pidió explícitamente.
...
**Lo que la caja pide explícitamente, cuenta.** Si dice «justifica en dos frases» y
la respuesta acierta pero no justifica, eso **no es «logrado»**: es «parcial». Lo
mismo si pide dos cosas y llega una —«nómbralos y di cuántos son», «pega las dos
respuestas»—, o si pide algo completo y llega un pedazo. Acertar no es lo mismo
que responder lo que se pidió, y dar por bueno lo segundo le enseña al alumno que
las instrucciones son decorativas.
...
**Y si la caja le pide nombrar algo, no lo nombres tú.** Es la forma más fácil de
soplar sin darse cuenta, porque no parece una explicación: parece una referencia.
Si la pregunta es «¿qué hay en la tercera parte del token?», no escribas la palabra
que la contesta —ni de pasada, ni enumerando lo que imprimió un comando, ni al
decirle en qué se equivocó—. Dile dónde volver a mirarlo y para ahí. Lo mismo con
un valor, un nombre de claim, un comando o un número que la caja pida averiguar.
Esto no aplica cuando **el alumno ya lo nombró bien**: ahí puedes confirmárselo.
 ...
 */
export async function revisar({ lab, cajaId, respuesta, pauta, modelo }) {
  const instruccion = armarInstruccion(lab, cajaId, respuesta, pauta);
  const opciones = {
    instruccion,
    esquema: ESQUEMA,
  ...
  /**
   * Una llamada, con la respuesta ya comprobada.
   *
   * Que el veredicto sea uno de los tres lo garantiza el `enum` del esquema. Lo
   * que **no** garantiza nadie es que el mensaje traiga texto: salió una vez un
   * «incompleto» con el mensaje en blanco, y un recuadro vacío bajo un «Vuelve a
   * mirarlo» es peor que no haber pedido nada. Así que un mensaje vacío se trata
   * igual que un JSON roto: se reintenta, y si vuelve vacío se falla.
   */
  const pedir = async () => {
    const r = await completar(opciones);
    const mensaje = (r.datos?.mensaje ?? '').trim();
    if (!mensaje) throw new Error('El modelo devolvió un veredicto sin mensaje');
    return { ...r, mensaje };
  };
  ...
  const { datos, uso, modelo: usado, mensaje } = r;
...
/**
 * La llave de caché: el texto del alumno **más el enunciado de la caja**, y la
 * pauta si la hay.
 *
 * Con el texto solo, un enunciado editado dejaría la sugerencia vieja pegada a una
 * pregunta que ya no es la misma. Y lo mismo vale para la pauta: si el docente
 * corrige el criterio, las sugerencias que se dieron con el criterio viejo tienen
 * que volver a pedirse.
 *
 * El separador es un NUL porque no puede aparecer en ninguna de las tres partes:
 * con un espacio, mover una palabra de una a otra daría la misma huella.
 *
 * Y la pauta se suma **solo cuando existe**, en vez de dejar el separador puesto:
 * así las huellas de los laboratorios que no tienen pauta —L0, L1, X1— siguen
 * siendo las mismas de antes, y nadie paga una revisión de nuevo por una función
 * que no le cambió nada.
 */
export async function huella(respuesta, enunciado, pauta) {
  const { createHash } = await import('node:crypto');
  const base = `${(respuesta ?? '').trim()}\0${enunciado ?? ''}`;
  return createHash('sha256')
    .update((pauta ?? '').trim() ? `${base}\0${pauta.trim()}` : base)
    .digest('base64url')
    .slice(0, 22);
}
